...
def dart_synthesis_of_elements(map_of_elements, element_key, dart_hive_type_ids):
    ...
    element_text = _get_dart_text_of_element(
        element_definition, dart_hive_type_ids)
...
def _get_dart_text_of_element(element_definition, dart_hive_type_ids):
    ...
    if (element_definition_enum != None):
        element_text = element_text + \
            _get_text_of_enum_definition(
                element_definition_enum, use_hive, dart_hive_type_ids)
    else:
        ...
        if (class_type_definition_unions != None):
            ...
            if (main_class_name != None):
                element_text = replace_all(
                    element_text, main_class_name, main_class_name + 'MainUnionType')
            ...
            if (main_class_name != None):
                unions_to_create_object.append(
                    main_class_name + 'MainUnionType')
            ...
            for index, union in enumerate(unions_to_create_object):
                ...
                serialization_text = serialization_text + \
                    '      Tuple2(this.' + is_union_object_function_name + \
                    ', this.' + as_union_object_function_name + '),\n'
            ...
            constructor_text = '  ' + union_class_name + '.fromValue({\n' + map_and_join_array(
                ...
                '    )!;\n' + \
                '  }\n'
            models_box_text = _get_models_boxes_text(
                use_hive, union_class_name, is_union=True)
        ...
        if (class_type_definition_intersections != None):
            ...
            if (union_class_name != None):
                element_text = replace_all(
                    element_text, union_class_name, union_class_name + 'MainIntersectionType')

            elif (main_class_name != None):
                element_text = replace_all(
                    element_text, main_class_name, main_class_name + 'MainIntersectionType')
            ...
            if (union_class_name != None):
                intersections_to_create_object.append(
                    union_class_name + 'MainIntersectionType')
            elif (main_class_name != None):
                intersections_to_create_object.append(
                    main_class_name + 'MainIntersectionType')
            ...
            for index, intersection in enumerate(intersections_to_create_object):
                ...
                deserialization_text = deserialization_text + \
                    '    this._values.add(' + intersection_object_name + \
                    '.fromJson(json));\n'
            ...
            models_box_text = _get_models_boxes_text(
                use_hive, intersection_class_name, is_intersection=True)

            constructor_text = '  ' + intersection_class_name + '.fromValues({\n' + map_and_join_array(
                subclasses_names,
                lambda x: '    required ' + x + ' ' +
                low_case_first_letter(x) + ',\n',
                lambda array: ''.join(array)
            ) + '  }){\n' + \
                '    _values = [\n' + map_and_join_array(
                subclasses_names,
...
def _get_text_of_enum_definition(enum_definition, use_hive, dart_hive_type_ids):
    ...
    if (use_hive):
        enum_text = enum_text + \
            '@HiveType(typeId: ' + \
            dart_hive_type_ids.get_hive_type_id(enum_definition.name) + ')\n'
    ...
    for option_index, option in enumerate(enum_definition.enum_options):
        ...
        if (use_hive):
            ...
            option_text = option_text + \
                '@HiveField(' + str(hive_number_field) + ')\n  '
...
def _get_text_of_type_field_definition(type_field_definition, father_class_name, use_hive, dart_hive_type_ids):
    ...
    if (enum_definition != None):
        ...
        additional_text = additional_text + \
            _get_text_of_enum_definition(
                enum_definition, use_hive, dart_hive_type_ids)
        ...
        additional_field_comment = 'additional type of field ' + \
            field_name + ' from class ' + father_class_name
